...
export function SimpleCardCarousel({
  ...
  return (
    <div className="mb-12">
      ...
      <div
        className="relative overflow-hidden rounded-xl bg-gray-900 py-8"
      ...
      >
        ...
        <div className="relative h-[300px] mx-auto">
          <div className="absolute inset-0 flex items-center justify-center">
            {visibleSlides.map(({ slide, position, isActive, isAdjacent, isEdge }) => {
              // 位置に基づいてスタイルを計算
              const translateX = position * 90; // カードの間隔
              ...
              return (
                <div
                  key={slide.id}
                  className="absolute w-[200px] transition-all duration-300 ease-in-out"
                  style={{
        ...
        {slides.length > 1 && (
          <>
            <button
              onClick={prevSlide}
              className="absolute left-2 top-1/2 -translate-y-1/2 z-20 bg-white/80 hover:bg-white text-gray-800 rounded-full p-2 shadow-md"
              aria-label="前のスライド"
            >
            ...
            <button
              onClick={nextSlide}
              className="absolute right-2 top-1/2 -translate-y-1/2 z-20 bg-white/80 hover:bg-white text-gray-800 rounded-full p-2 shadow-md"
              aria-label="次のスライド"
            >
        ...
        {/* インジケーター */}
        {slides.length > 1 && (
          <div className="absolute bottom-2 left-1/2 -translate-x-1/2 z-20 flex justify-center space-x-2">
            {slides.map((_, index) => (
              <button
                key={index}
